...
void main()
{
	...
	// 播放背景音乐
	mciSendString( "open C:\\Users\\zaoxiaohua\\Desktop\\hdh.mp3 alias bk", 0, 0, 0 );
	mciSendString( "play bk repeat", 0, 0, 0 );

	setfillstyle( 0	);
	setfont		( 36, 0, "楷体"	);
	setcolor	( YELLOW );
	outtextxy	( 370, 100, "木又，很高兴认识你.... ^_^" );
...
/**** 储存烟花的像素点颜色 ****/
	IMAGE fm, gm;
	loadimage( &fm, "C:\\Users\\zaoxiaohua\\Desktop\\flower.jpg", 3120, 240 );
...
/**** 加载烟花弹 ************/
	IMAGE sm;
	loadimage( &sm, "C:\\Users\\zaoxiaohua\\Desktop\\shoot.jpg", 200, 50 );
...
void Chose( DWORD& t1 )
{
	...
	if ( t2 - t1 > 100 )
	{
		...
		if ( n < 13 && Jet[n].shoot == false && Fire[n].show == false )
		{
			...
			sprintf( c1, "open C:\\Users\\zaoxiaohua\\Desktop\\shoot.mp3 alias s%d", n );
	...
	for ( int i = 0; i < 13; i++ )
	{
		...
		if ( Jet[i].t2 - Jet[i].t1 > Jet[i].dt && Jet[i].shoot == true )
		{
			...
			if ( Jet[i].y <= Jet[i].hy )
			{
				// 播放爆炸声
				char c1[50], c2[30], c3[30];
				sprintf( c1, "open C:\\Users\\zaoxiaohua\\Desktop\\bomb.wav alias n%d", i );
...
void Style( DWORD& st1 )
{
	...
	if ( st2 - st1 > 266000)		// 一首歌的时间
	{
		...
		for ( int i = 0; i < NUM; i++ )
		{
			...
			/**** 播放发射声音 ***/
			char c1[50], c2[30], c3[30];
			sprintf( c1, "open C:\\Users\\zaoxiaohua\\Desktop\\shoot.mp3 alias s%d", i );
